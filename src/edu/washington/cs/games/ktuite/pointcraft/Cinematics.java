...
public class Cinematics  {
	...
	public static void setupInterp() {
		...
		float y1 = -start_pos.z;
		float x2 = target_pos.x;
		float y2 = -target_pos.z;
		float dx1 = (float)Math.cos(start_tilt_angle);
		float dy1 = (float)Math.sin(start_tilt_angle);
		float mag1 = (float)Math.sqrt(dx1*dx1 + dy1*dy1);
		dx1 /= mag1; dy1 /= mag1;
		float dx2 = (float)Math.cos(target_tilt_angle);
		float dy2 = (float)Math.sin(target_tilt_angle);
		float mag2 = (float)Math.sqrt(dx2*dx2 + dy2*dy2);
		dx2 /= mag2; dy2 /= mag2;
		Vector3f solution = Cinematics.linearSolver((float)dx1, (float)-dx2, (float)(x2 - x1),
													(float)dy1, (float)-dy2, (float)(y2 - y1));
		
		float cx = x1 + solution.x * dx1;
		float cy = y1 + solution.x * dy1;
		...
		Cinematics.rotation_center.x = cx;
		Cinematics.rotation_center.y = cy;

		dx1 = x1 - cx;
		dy1 = y1 - cy;
		mag1 = (float)Math.sqrt((dx1*dx1) + (dy1*dy1));
		dx1 /= mag1; dy1 /= mag1;
		Cinematics.start_radius = mag1;
		Cinematics.start_theta = (float)Math.acos(dx1);
		
		dx2 = x2 - cx;
		dy2 = y2 - cy;
		mag2 = (float)Math.sqrt((dx2*dx2) + (dy2*dy2));
		dx2 /= mag2; dy2 /= mag2;
		Cinematics.target_radius = mag2;
		Cinematics.target_theta = (float)Math.acos(dx2);
		*/
	}
	...
	public static Vector3f linearSolver(float a1, float b1, float c1, float a2, float b2, float c2) {
		float d = a1*b2 - a2*b1;
		Vector3f v = new Vector3f((c1*b2 - c2*b1)/d, (a1*c2 - a2*c1)/d, 0.0f);
		return v;
	}
