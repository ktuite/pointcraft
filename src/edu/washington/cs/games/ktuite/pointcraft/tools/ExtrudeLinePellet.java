...
public class ExtrudeLinePellet extends Pellet {
	 ...
	 */
	public ExtrudeLinePellet() {
		super();
		pellet_type = Main.GunMode.EXTRUDE_LINE;
	}

	public ExtrudeLinePellet(Pellet lp) {
		super();
		pos.set(lp.pos);
		radius = lp.radius;
	...
	private static void makeFrontFaceOfBox() {
		...
		Pellet d = new ExtrudeLinePellet(user_pellets.get(0));
	...
	private static void extrudeBox() {
		...
		for (int i = 0; i < 4; i++){
			Pellet extruded = new ExtrudeLinePellet(pellets.get(i));
