...
public class BoxPellet extends Pellet {
	...
	public BoxPellet() {
		super();
		pellet_type = Main.GunMode.BOX;
	}

	public BoxPellet(Pellet lp) {
		super();
		pos.set(lp.pos);
		radius = lp.radius;
	...
	private static void makeFrontFaceOfBox() {
		...
		Pellet d = new BoxPellet(user_pellets.get(0));
	...
	private static void extrudeBox() {
		...
		for (int i = 0; i < 4; i++){
			Pellet extruded = new BoxPellet(pellets.get(i));
