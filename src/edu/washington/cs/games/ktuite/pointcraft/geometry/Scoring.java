...
public class Scoring {
	...
	private static boolean pointInTri(double x, double y, double z,
			double ax, double ay, double az,
			double bx, double by, double bz,
			double cx, double cy, double cz,
			double epsilon)
	{
	    Vector2f bc = barycentricCoords(x, y, z, ax, ay, az, bx, by, bz, cx, cy, cz);
	...
	public static double computeTextureScore(Primitive p)
	{
		...
		for (int i = 0; i < PointStore.num_points; i++) {
			...
			if (pointInTri(v.x, v.y, v.z, a.x, a.y, a.z, b.x, b.y, b.z, c.x, c.y, c.z, epsilon)) {
				count++;
			}
		}
